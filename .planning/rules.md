...
2. **`PACKAGE_STANDARDS.md` is normative.** The decision table at its top wins over any other doc. pnpm + Node ≥18 only (no Bun); pnpm is pinned via the root `packageManager` field (CI's `pnpm/action-setup` reads it — no `version:` input). Flat `dist/` via tsup, `noEmit: true` tsconfigs, zod peerDep ≥4.4.0.
...
6. **Versioning via Changesets** — every user-visible change ships with a changeset. **Never hand-edit `version` fields or `CHANGELOG.md`;** only the changesets release PR (`changeset version`) may change them, and that flow deletes the consumed changeset files. A changeset left on `main` after its release shipped triggers a duplicate / double-bump release PR (see I-004).
