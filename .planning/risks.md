...
|---|---|---|---|---|
| R-1 | Zod minor versions change `_zod` internals, breaking the structural types | Medium | Pin peer range (`>=4.4 <5`); add CI canary compiling structural types against newest Zod | 🔄 peer range pinned; canary → tasks.md T-018 |
| R-2 | Standards docs drift apart | High (already happened once) | One normative doc (rule 2) + conformance enforced in CI (rule 9) | 🔄 single doc done; conformance script → tasks.md T-019 |
